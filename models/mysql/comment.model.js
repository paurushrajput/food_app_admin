  ...
  class CommentModel {
    ...
    static async getComment(body) {
      ...
      let query = `SELECT f.id,f.uid,f.post_type,f.post_id,f.user_id,
        f.created_at FROM ${Tables.COMMENT} as f WHERE f.post_id = $1 AND f.post_type = $2  ${pagination}`
      const statement = {
        text: query,
        values: values,
        rowsOnly: true,
      };
      let countQuery = `SELECT count(*) as comments_count  
      FROM ${Tables.COMMENT} as f WHERE f.post_id = $1 AND f.post_type = $2 `
      ...
      return {
        count: promiseData[1]?.value?.rows[0]?.comments_count,
